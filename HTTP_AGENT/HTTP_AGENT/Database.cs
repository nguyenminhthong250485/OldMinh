...
namespace HTTP_AGENT
{
    public class Database
    {
        ...
        public DataTable GetParnerData(string ParnerName)
        {
            return execQuery("select * from AgentIbet where ParnerName='" + ParnerName + "'").Tables[0];
        }
        public DataTable GetData(string Bookie)
        {
            if (Bookie == "Ibet")
            {
                return execQuery("select * from AgentIbet").Tables[0];
            }
            else
            {
                return execQuery("select * from AgentSbo").Tables[0];
            }
        }
        ...
        public int doInsertParnerName(string name)
        {
            return execNonQuery("insert into ParnerName values('" + name.Replace("'", "''") + "')");
        }
    }
}
