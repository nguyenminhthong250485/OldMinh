...
namespace BetList
{
    public class Database
    {
        ...
        public DataTable getBetList()
        {
            ...
            return execQuery(@"SELECT top 100 [id]
	                          ,phieuchung
                              ,[type]
                              ,[member]
                              ,[hdp]
        ...
        public DataTable getTBetList()
        {
            ...
            return execQuery(@"SELECT top 100 [id]
                              ...
                              ,[group]
                              ,[systemdate]
                          FROM [Humbuger].[dbo].[vTBetList]
                          where systemdate between '" + startdate.Replace("'", "''") + "' and '" + enddate.Replace("'", "''") + " 23:59:59' order by id desc").Tables[0];
        }
        public DataTable getCompare()
        {
            return execQuery(@"SELECT [id]    
                  ,[CompareLive]                
                  ,[CompareNonLive]
                FROM CompareAccount where id='a,a'").Tables[0];
        }

        public DataTable getUnder()
        {
            return execQuery(@"SELECT [id]    
                  ,[CompareLive]                
                  ,[CompareNonLive]
                FROM CompareAccount where id='under'").Tables[0];
        }

        public DataTable getMinus(String filter = "")
        {
            if (filter == "")
                return execQuery(@"SELECT [id]
                                  ,[keo]
                ...
                return execQuery(@"SELECT [id]
                                  ...
                                  ,[data2]
                                  ,[systemdate]
                              FROM [Humbuger].[dbo].[Minus] where DATEDIFF(s,'1970-1-1',GETDATE())-DATEDIFF(s,'1970-1-1',systemdate) <= 43200 and id in (" + filter + ")").Tables[0];
        }
    }
}
