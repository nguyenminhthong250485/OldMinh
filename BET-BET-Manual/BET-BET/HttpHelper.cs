...
namespace BET_BET
{
    public class HttpHelper
    {
        ...
        public string Fetch(string url, HttpMethod method, string referer, string postData, string ip = "", string Ori = "", string Host = "")
        {
            string r = string.Empty;
            try
            {
                using (HttpWebResponse res = FetchResponse(url, method, referer, postData, ip, Ori, Host))
                {
                    if (res != null)
                    {
